...
function FromToPage() {
  ...
  const getFromToFields = () => {
    setLoading(true);
    axios.get(`http://192.168.1.8/api/Bus/buses`)
      .then(res => {
  ...
  const formik = useFormik({
    ...
    onSubmit: values => {
      ...
      //  navigate.push(`/booking?from=$(values.from)&to=$(values.to)&date=$(values.date)`)
       navigate(`/booking?from=${values.from}&to=${values.to}&date=${values.date}`);
    //    ({
    //   pathname: '/booking',
      ...
      <form onSubmit={formik.handleSubmit} >
        <label>
          ...
          <Select
            ...
            label="From"
            isSearchable={true}
            onChange={(option) => formik.setFieldValue("from", option.cityName)}
            options={fromToFields}
            value={fromToFields.filter((option) => option.cityName === formik.values.from)}
          />
        </label>
        ...
        <label>
          ...
          <Select
            ...
            value={fromToFields.filter((option) => option.cityName === formik.values.to)}
            options={fromToFields}
          />
        </label>
